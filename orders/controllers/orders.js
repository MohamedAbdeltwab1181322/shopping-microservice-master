...
const createOrder = async(reqData) => {
    try {
        ...
        const response = await orderModel.createOrder(validInput, reqData.user._user_id);
        await producer(validInput, response[0], 'order');
        return successResponse(201, { product_id: validInput.product_id, id: response[0] })
    } catch (error) {
...
const fetchOrder = async(reqData) => {
    try {
        ...
        return consumer(`${Math.random()}-group`, validInput.id, 'order').then(({ message }) => successResponse(200, JSON.parse(message.value.toString())));
